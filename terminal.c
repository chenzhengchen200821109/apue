...
int tty_cbreak(int fd) /* put terminal into cbreak mode */
{
    ...
    /*
     * Verify that the change stuck. tcsetattr can return 0 on partial success.
     */
    if (tcgetattr(fd, &buf) < 0) {
        err = errno;
...
int tty_raw(int fd) /* put terminal into a raw mode */
{
    ...
    if ((buf.c_lflag & (ECHO | ICANON | IEXTEN | ISIG)) || (buf.c_iflag & (BRKINT | ICRNL | INPCK | ISTRIP | IXON)) ||
            (buf.c_cflag & (CSIZE | PARENB | CS8)) != CS8 || (buf.c_oflag & OPOST) || buf.c_cc[VMIN] != 1 || buf.c_cc[VTIME] != 0) {
        tcsetattr(fd, TCSAFLUSH, &save_termios);
        errno = EINVAL;
        return (-1);
    }
