 ...
 * These are used to construct records in the index file and data file.
 */
...
/*
 * The following definitions are for hash chains and free list chain in 
 * the index file.
 */
#define PTR_SZ 7 /* size of ptr field in hash chain */
#define PTR_MAX 999999 /* max file offset = 10**PTR_SIZE - 1 */
...
/*
 * Open or create a database. Same argument as open(2).
 */
DBHANDLE db_open(const char *pathname, int oflag, ...)
{
	...
	if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)) {
		...
		if (statbuf.st_size == 0) {
			/*
			 * We have to build a list of (NHASH_DEF + 1) chain ptrs with
			 * a vlaue of 0. The +1 is for the free list pointer that preceedes
			 * the hash table.
			 */
			sprintf(asciiptr, "%*d", PTR_SZ, 0);
			hash[0] = 0; /* null-terminated string set for strcat() */
			for (i = 0; i < NHASH_DEF + 1; i++)
				strcat(hash, asciiptr);
			strcat(hash, "\n");
			i = strlen(hash);
			if (write(db->idxfd, hash, i) != (ssize_t)i)
...
/*
 * Free up a DB structure, and all the malloc'ed buffers it may
 * point to. Also close the file descriptors if still open.
 */
...
char *db_fetch(DBHANDLE h, const char *key)
{
	...
	if (_db_find_and_lock(db, key, 0) < 0) {
		...
		ptr = _db_readdat(db); /* return a pointer to data */
		db->cnt_fetchok++;
	}
...
static int _db_find_and_lock(DB *db, const char *key, int writelock)
{
	 ...
	 * of corresponding chain ptr in hash table. This is where our search starts.
	...
	/*
	 * We lock the hash chain here. The caller must unlock it when done. Not we lock
	 * and unlock only the first byte.
	 */
	if (writelock) {
	...
	/*
	 * Get the offset in the index file of first record on the hash chain (can be zero).
	 */
 ...
 * the index file. We read the index record into db->idxbuf and replace
 * the seperators with null bytes. If all is OK we set db->datoff and db->datlen
 * to the offset and length of the corresponding data record in the data file.
 */
static off_t _db_readidx(DB *db, off_t offset)
{
	 ...
	 * us with offset==0, meaning read from current offset. We still
	 * need to call lseek to record the current offset.
	 */
	if ((db->idxoff = lseek(db->idxfd, offset, offset == 0 ? SEEK_CUR : SEEK_SET)) == -1)
		err_dump("_db_readidx: lseek error");

	/*
	 * Read the ascii chain ptr and the ascii length at the front
	 * of the index record. This tells us the remaining size of the
	 * index record.
	 */
	 ...
	 /*
	  * Now read the actual index record. We read it into the key buffer that we
	  * malloced when we opened the database.
	  */
	 if ((i = read(db->idxfd, db->idxbuf, db->idxlen)) != (ssize_t)db->idxlen)
		err_dump("_db_readidx: read error of index record");
	 if (db->idxbuf[db->idxlen - 1] != NEWLINE) /* sanity check */
		err_dump("_db_readidx: msssing newline");
	 ...
	 *ptr1++ = 0; /* replace SEP with null */
	 if ((ptr2 = strchr(ptr1, SEP)) == NULL)
		err_dump("_db_readidx: missing second seperator");
	 *ptr2++ = 0;
	 if (strchr(ptr2, SEP) != NULL)
		err_dump("_db_readidx: too many seperators");
...
static char *_db_readdat(DB *db)
{
	...
	if (read(db->datfd, db->datbuf, db->datlen) != (ssize_t)db->datlen)
 ...
 * This function is called by db_delete and db_store, after the record
 * has been located by _db_find_and_lock.
 */
static void _db_dodelete(DB *db)
{
	...
	for (ptr = db->datbuf, i = 0; i < (int)(db->datlen - 1); i++)
		*ptr++ = SPACE;
	*ptr = 0; /* null terminate for _db_wrtiedat */
	ptr = db->idxbuf;
	while (*ptr)
		*ptr++ = SPACE;
	...
	/*
	 * Save the contents of index record chain ptr, before it's 
	 * rewritten by _db_writeidx.
	 */
	saveptr = db->ptrval;

	/*
	 * Rewrite the index record. This also rewrites the length of the index record,
	 * the data offset, and the data length, none of which has changed, but that's OK.
	 */
	...
	/*
	 * Rewrite the chain ptr that pointed to this record being deleted. Recall that
	 * _db_find_and_lock sets db_ptroff to point to this chain ptr. We set this chain
	 * ptr to the contents of the deleted record's chain ptr, saveptr.
	 */
...
/*
 * Write a data record. Called by _db_dodelete (to write the record with blanks)
 * and db_store.
 */
static void _db_writedat(DB *db, const char *data, off_t offset, int whence)
{
	 ...
	 * to make the two an atomic operation. If we're overwriting an existing
	 * record, we don't have to lock.
	 */
...
/*
 * Write an index record. _db_writedat is called before this function to set the
 * datoff and datlen fields in the DB structure, which we need to write the index record.
 */
static void _db_writeidx(DB *db, const char *key, off_t offset, int whence, off_t ptrval)
{
	...
	/*
	 * If we're appending, we have to lock before doing the lseek and write to make the two
	 * an atomic operation. If we're overwriting an existing record, we don't have to lock.
	 */
...
/*
 * Write a chain ptr field somewhere in the index file: the free list, the hash table, or
 * in an index record.
 */
...
/*
 * Store a record in the database. Return 0 if OK, 1 if record exists and DB_INSERT
 * specified, -1 on error.
 */
int db_store(DBHANDLE h, const char *key, const char *data, int flag)
{
	 ...
	 * following calls to _db_writeptr change the hash table entry for this chain
	 * to point to the new record. The new record is added to the front of the
	 * hash chain.
	 */
	if (_db_find_and_lock(db, key, 1) < 0) { /* record not found */
		...
		/*
	 	* _db_find_and_lock locked the hash chain for us; read the chain ptr to
	 	* the first index record on hash chain.
	 	*/
		ptrval = _db_readptr(db, db->chainoff);

		if (_db_findfree(db, keylen, datlen) < 0) {
			/*
			 * Can't find an empty record big enough. Append the new record
			...
			/*
			 * Reuse an empty record. _db_findfree removed it from the free list
			 * and set both db->datoff and db->idxoff. Reused record goes to the
			 * front of the hash chain.
			 */
		...
		/* 
		 * We are replacing an existing record. We know the new key equals the 
		 * existing key, but we need to check if the data records are the 
		 * same size.
		 */
		if (datlen != (int)db->datlen) {
			...
			/*
			 * Reread the chain ptr in the hash table (it may change with the deletion).
			 */
			ptrval = _db_readptr(db, db->chainoff);
...
/*
 * Try to find a free index record and accompanying data record of the
 * correct sizes. We're only called by db_store.
 */
static int _db_findfree(DB *db, int keylen, int datlen)
{
	...
	while (offset != 0) {
		nextoffset = _db_readidx(db, offset);
		if ((int)strlen(db->idxbuf) == keylen && (int)db->datlen == datlen)
			break;
		saveoffset = offset;
		offset = nextoffset;
	}
	...
	if (offset == 0) {
		...
		/*
		 * Found a free record with matching sizes. The index record was read in by
		 * _db_readidx above, which sets db->ptrval. Also, saveoffset points to the 
		 * chain ptr that pointed to this empty record on the free list. We set this
		 * chain ptr to db->ptrval, which removes the empty record from the free list.
		 */
		_db_writeptr(db, saveoffset, db->ptrval);
		rc = 0;

		/*
		 * Notice also that _db_readidx set both db->idxoff and db->datoff. This is used
		 * by the calller, db_store, to write the new index record and data record.
		 */
	}
...
/*
 * Rewind the index file for _db_nextrec. Automatically called by db_open.
...
void db_rewind(DBHANDLE h)
{
	...
	/*
	 * we're just setting the file offset for this process to the start
	 * of the index records;; no need to lock. +1 below for newline at 
	 * end of hash table.
	 */
...
/*
 * Return the next sequential record. We just step our way through the index file,
 * ignoring deleted records. db_rewind must be called before this function is called
 * the first time.
 */
char *db_nextrec(DBHANDLE h, char *key)
{
	...
	/*
	 * We read lock the free list so that we don't read a record in the middle of its
	 * being deleted.
	 */
