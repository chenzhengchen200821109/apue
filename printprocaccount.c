...
#else
#define FMT "%-*.*s etime = %f, chars = %7ld, stat = %3u: %c %c %c %c\n"
#endif
...
int main(int argc, char *argv[])
{
    ...
    printf("size of struct acct = %d\n", sizeof(struct acct));
    printf("size of struct acct_v3 = %d\n", sizeof(struct acct_v3));
    while (fread(&acdata, sizeof(acdata), 1, fp) == 1) {
        printf(FMT, (int)sizeof(acdata.ac_comm), (int)sizeof(acdata.ac_comm), acdata.ac_comm,
#if defined(BSD)
                acdata.ac_etime, acdata.ac_io
#else
                acdata.ac_etime, compt2ulong(acdata.ac_io),
#endif
#if defined(HAS_AC_STAT)
                (unsigned char)acdata.ac_stat,
#else
                (unsigned int)acdata.ac_exitcode,
#endif
