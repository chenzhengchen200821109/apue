/*
 * This program demonstrates the necessarity of using recursice mutex.
 */
...
void *timeout_helper(void *arg)
{
    ...
    (*tip->to_fn)(tip->to_arg); /* call a function */
    free(arg);
    return(0);
}
...
void timeout(const struct timespec *when, void (*func)(void *), void *arg)
{
    ...
    if ((when->tv_sec > now.tv_sec) ||
        ...
        if (tip != NULL) {
            ...
            err = make_detach_thread(timeout_helper, (void *)tip);
            if (err == 0)
                return;
            else
                free(tip);
...
int main(void) 
{
    ...
    /*
     * Check the condition under the protection of a lcok to make
