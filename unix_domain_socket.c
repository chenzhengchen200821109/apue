 ...
 * We also obtain the client's user ID from the pathname that it
 * must bind before calling us.
...
int serv_accept(int listenfd, uid_t *uidptr)
{
    ...
    if ((clifd = accept(listenfd, (struct sockaddr *)&un, &len)) < 0) {
        free(name);
        return (-2); /* often errno=EINTR, if signal caought */
    }
 ...
 * Returns fd if all OL, <0 on error.
 */
